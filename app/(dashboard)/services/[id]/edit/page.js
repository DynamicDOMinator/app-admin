    ...
    const fetchService = async () => {
      try {
        ...
        setFormData({
          ...
          notice: s.notice || '',
          noticeAr: s.noticeAr || '',
          includedTasksAr: s.includedTasksAr || [],
          equipmentNoticeAr: s.equipmentNoticeAr || '',
          ...
          roomOptions: (s.roomOptions || []).map(item => ({ ...item, key: item.key || item._id || Date.now().toString() + Math.random() })),
          areaOptions: (s.areaOptions || []).map(item => ({ ...item, key: item.key || item._id || Date.now().toString() + Math.random() })),
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto pt-2 sm:pt-4">
                <button
                  ...
                  onClick={() => {
                    ...
                    toast.success(`تم إنشاء قسم (${newCatInput.trim()}) بنجاح`);
                  }}
